...
public class ShortWeekView<
    [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] T> : DayWeekViewBase<T>
    where T : CalendarItem
{
    protected override int DaysInView => 3;

    protected override List<CalendarCell<T>> BuildCells()
    {
        var cells = new List<CalendarCell<T>>();
        var range = new CalendarDateRange(Calendar.CurrentDay.Date, CalendarView.ShortWeek, Calendar.Culture, Calendar.GetFirstDayOfWeekByCalendarView(CalendarView.ShortWeek));
